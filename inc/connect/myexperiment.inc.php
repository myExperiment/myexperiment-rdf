...
if (!empty($myexp_db['password'])) mysql_connect($myexp_db['server'],$myexp_db['user'],$myexp_db['password']) or die("Could not connect to MySQL on {$myexp_db['server']} as user {$myexp_db['user']} WITH password.");
else  mysql_connect($myexp_db['server'],$myexp_db['user']) or die("Could not connect to MySQL on {$myexp_db['server']} as user {$myexp_db['user']} WITHOUT password.");
